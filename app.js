...
const DAPURS_DEFAULT=[
  ...
  {id:'dapur-5',       name:'Dapur 5',       icon:'🍜', pattern:'pattern-a', accent:'#fb7185', bg:'#3d0a14'},
];
...
function toggleTheme(){
  ...
  applyThemeMode(cur==='light'?'dark':'light');
}
...
function loadTheme(){applyThemeMode(localStorage.getItem('kok_theme')||'dark');}
...
function orderDay(tglStr){
  const m=tglStr.match(/(\d{1,2})\s+(\w+)\s+(\d{4})/);if(!m)return'';
  const bi=BULAN.findIndex(b=>b.toLowerCase()===m[2].toLowerCase());if(bi<0)return'';
  const dt=new Date(parseInt(m[3]),bi,parseInt(m[1]));dt.setDate(dt.getDate()-1);
...
function getDaysUntil(tglStr){
  ...
  const m=tglStr.match(/(\d{1,2})\s+(\w+)\s+(\d{4})/);if(!m)return null;
  const bi=BULAN.findIndex(b=>b.toLowerCase()===m[2].toLowerCase());if(bi<0)return null;
  const target=new Date(parseInt(m[3]),bi,parseInt(m[1]));target.setHours(0,0,0,0);
...
let STOK_KOPERASI=[];
let curWeekId=null, curTab='dashboard-dapur';
let WD={days:[]};
...
const DEFAULT_WD={days:[
  {n:"Selasa, 02 Juni 2026",od:"Senin, 01 Juni 2026",os:"Senin 01/06",items:[
    ...
    {b:"Pisang Berlin",q:4800,s:"pcs",h:850,hb:null,ket:""},{b:"Bawang Putih",q:2,s:"kg",h:30000,hb:null,ket:""},
  ]},
...
function saveLocal(){
  ...
  try{localStorage.setItem('kok_koperasi_stok',JSON.stringify(STOK_KOPERASI));}catch(e){}
}
function loadLocal(dapurId){
  ...
  try{STOK_KOPERASI=JSON.parse(localStorage.getItem('kok_koperasi_stok')||'[]');}catch(e){STOK_KOPERASI=[];}
}
...
async function dbLoadKoperasiStok(){
  try{
    const r=await sbG('settings','key=eq.koperasi_stok');
    if(r[0]&&Array.isArray(r[0].value))STOK_KOPERASI=r[0].value;
  }catch(e){}
  try{localStorage.setItem('kok_koperasi_stok',JSON.stringify(STOK_KOPERASI));}catch(e){}
}
async function dbSaveKoperasiStok(){
  try{await sbU('settings',{key:'koperasi_stok',value:STOK_KOPERASI,updated_at:new Date().toISOString()});}catch(e){}
  try{localStorage.setItem('kok_koperasi_stok',JSON.stringify(STOK_KOPERASI));}catch(e){}
}
...
function openDapur(dapurId){
  const cfg=DAPURS.find(d=>d.id===dapurId);if(!cfg)return;
  ...
  document.getElementById('dapur-sub').textContent=WD.days[0]?fmtTgl(WD.days[0].n):'Belum ada data';
  if(!topbar._tabsInit){
...
function resetToBaseTheme(mode){
  ...
  ['--bg','--bg2','--bg3','--surface','--surface2','--accent'].forEach(p=>r.removeProperty(p));
  if(mode==='light') document.documentElement.setAttribute('data-theme','light');
  else document.documentElement.removeAttribute('data-theme');
}
function goBack(){
...
function goSettings(){showPage('settings');renderSettings();}
function applyDapurTheme(cfg){
  document.documentElement.style.setProperty('--dapur-accent',cfg.accent||'#6366f1');
...
function switchTab(name){
  ...
  const map={ringkasan:renderRingkasan,input:()=>renderInput(false),pesan:renderOrderH,checklist:renderCL,rekap:renderRekap,stok:renderStok,histori:rHist};
...
function renderDashboardStok(){
  const cont=document.getElementById('dash-stok-c');if(!cont)return;
  cont.innerHTML='';
  // Koperasi stok (global)
  if(STOK_KOPERASI.length>0){
    const kop=document.createElement('div');kop.className='stok-entity-block';
    kop.innerHTML=`<div class="stok-entity-title">🏪 Koperasi (Stok Sisa)</div>
      <div style="overflow-x:auto"><table class="stok-entity-table">
        <thead><tr>
        ...
        <tbody>${STOK_KOPERASI.map(it=>`<tr>
          <td style="font-weight:500">${escH(it.b)}</td>
          <td class="se-num">${fNum(it.jml)}</td>
          <td class="se-sat">${escH(normSat(it.s||''))}</td>
        </tr>`).join('')}</tbody>
      </table></div>`;
    cont.appendChild(kop);
  }
  // Stok per-dapur
  const allItems={};
  DAPURS.forEach(d=>{
  ...
  DAPURS.forEach(entity=>{
    const hasData=items.some(it=>it.perDapur[entity.id]>0);
    if(!hasData)return;
    const entityItems=items.filter(it=>it.perDapur[entity.id]>0);
    const wrap=document.createElement('div');wrap.className='stok-entity-block';
    wrap.innerHTML=`<div class="stok-entity-title">${entity.icon} ${entity.name}</div>
      <div style="overflow-x:auto"><table class="stok-entity-table">
        <thead><tr>
          <th style="text-align:left;min-width:120px">Bahan</th>
          <th style="text-align:right;min-width:80px">Jumlah</th>
        ...
        <tbody>${entityItems.map(it=>`<tr>
          <td style="font-weight:500">${escH(it.name)}</td>
          <td class="se-num">${fNum(it.perDapur[entity.id])}</td>
          <td class="se-sat">${escH(normSat(it.sat||''))}</td>
        </tr>`).join('')}</tbody>
      </table></div>`;
    cont.appendChild(wrap);
  });
  if(!cont.children.length)cont.innerHTML='<p class="empty" style="padding:1.5rem 0">Belum ada data stok.</p>';
}
...
function renderDashboard(){
  ...
  DAPURS.forEach(d=>{
    try{
      const s=JSON.parse(localStorage.getItem(`kok_${d.id}`)||'{}');
      const wd=s.WD;if(!wd)return;const ck=s.CK||{};
      wd.days.forEach((day,di)=>{
        const daysUntil=getDaysUntil(day.n);if(daysUntil===null)return;
        day.items.forEach((item,ii)=>{
          if(ck[`${di}_${ii}`])return;
          const duO=daysUntil-1;
          if(isPisang(item.b)&&duO<=4&&duO>0) alerts.push({type:'h4',dapur:d.name,icon:d.icon,bahan:item.b,qty:item.q,sat:item.s});
          else if(!isPisang(item.b)&&duO===1)  alerts.push({type:'h1',dapur:d.name,icon:d.icon,bahan:item.b,qty:item.q,sat:item.s});
          else if(!isPisang(item.b)&&duO===2)  alerts.push({type:'h2',dapur:d.name,icon:d.icon,bahan:item.b,qty:item.q,sat:item.s});
        });
      });
    }catch(e){}
  });
  const strip=document.getElementById('alert-strip');strip.innerHTML='';
  const typeConfig={h1:{icon:'🔴',label:'H-1 — Harus diorder HARI INI',cls:'h1'},h2:{icon:'🟠',label:'H-2 — Perlu diorder besok',cls:'h2'},h4:{icon:'🍌',label:'H-4 Pisang — Pesan sekarang',cls:'h4'}};
  ['h1','h2','h4'].forEach(type=>{
    const group=alerts.filter(a=>a.type===type);if(!group.length)return;
    const cfg=typeConfig[type];
    const byDapur={};
    group.forEach(a=>{if(!byDapur[a.dapur])byDapur[a.dapur]={icon:a.icon,items:[]};byDapur[a.dapur].items.push(`${a.bahan}${a.qty?' '+fNum(a.qty)+' '+(a.sat||''):''}`);});
    const div=document.createElement('div');div.className='alert-item '+cfg.cls;
    div.innerHTML=`<div class="alert-icon">${cfg.icon}</div>
      <div class="alert-body">
        <div class="alert-title">${cfg.label}</div>
        <div class="alert-breakdown">${Object.entries(byDapur).map(([name,d])=>`<div class="alert-dapur-row">${d.icon} <strong>${name}:</strong> ${d.items.join(', ')}</div>`).join('')}</div>
      </div>
  ...
  // Check H-1 notif
  if(alerts.filter(a=>a.type==='h1').length>0) checkAndNotify(alerts);
  ...
  DAPURS.forEach(d=>{
      ...
      if(wd)wd.days.forEach((day,di)=>{
        ...
        day.items.forEach((item,ii)=>{
          ...
          if(!ck[`${di}_${ii}`]){
            ...
            if(isPisang(item.b)&&duO!==null&&duO<=4&&duO>0)pisang++;
            if(!isPisang(item.b)&&duO===1)h1++;
            if(!isPisang(item.b)&&duO===2)h2++;
          }
        });
      });
    }catch(e){}
    ...
    card.innerHTML=`
      <div class="dc-header ${d.pattern}" style="background:linear-gradient(135deg,${d.bg} 0%,${d.bg}bb 100%)">
        ...
        <button class="dc-rename-btn" onclick="event.stopPropagation();renameDapurCard('${d.id}')" title="Ganti nama">✏️</button>
      </div>
      <div class="dc-prog-bar"><div class="dc-prog-fill" style="width:${pct}%;background:${d.accent}"></div></div>
      <div class="dc-body">
        <div class="dc-stat"><div class="dc-stat-val ${isAllDone?'green':''}">${dn}/${tot}</div><div class="dc-stat-lbl">Diorder</div></div>
        <div class="dc-stat"><div class="dc-stat-val ${h1>0?'red':''}">${h1}</div><div class="dc-stat-lbl">H-1</div></div>
        <div class="dc-stat"><div class="dc-stat-val ${h2>0?'orange':''}">${h2}</div><div class="dc-stat-lbl">H-2</div></div>
        <div class="dc-stat"><div class="dc-stat-val ${pisang>0?'yellow':''}">${pisang}</div><div class="dc-stat-lbl">🍌 H-4</div></div>
      </div>
      <div class="dc-footer">
...
function renameDapurCard(id){
  const d=DAPURS.find(x=>x.id===id);
  const newName=prompt('Ganti nama dapur:', d?.name||id);
  if(newName&&newName.trim()){
    saveDapurName(id,newName.trim());
    renderDashboard();
    toast('✓ Nama disimpan');
  }
}
...
async function requestNotifPermission(){
  ...
  if(perm==='granted'){
    ...
    try{localStorage.setItem('kok_notif','on');}catch(e){}
  } else {
    toast('Notifikasi diblokir oleh browser');
    try{localStorage.setItem('kok_notif','off');}catch(e){}
  }
  renderSettings();
}
function checkAndNotify(alerts){
  if(Notification.permission!=='granted')return;
  if(localStorage.getItem('kok_notif')!=='on')return;
  const h1=alerts.filter(a=>a.type==='h1');
  if(!h1.length)return;
  // Debounce: hanya tampilkan sekali per sesi
  if(sessionStorage.getItem('notif_shown'))return;
  sessionStorage.setItem('notif_shown','1');
  const byDapur={};
  h1.forEach(a=>{if(!byDapur[a.dapur])byDapur[a.dapur]=[];byDapur[a.dapur].push(a.bahan);});
  const body=Object.entries(byDapur).map(([d,items])=>`${d}: ${items.slice(0,3).join(', ')}`).join('\n');
  try{new Notification('⚠️ H-1 — Order Sekarang!',{body,icon:'/favicon.ico'});}catch(e){}
}
...
function switchTabToDay(tabName,orderDay){
  ...
  setTimeout(()=>{
    if(!orderDay)return;
    const shortName=orderDay.split(',')[0].trim();
    document.querySelectorAll('.od-title').forEach(el=>{
      if(el.textContent.includes(shortName)||el.textContent.includes(orderDay)){
        el.scrollIntoView({behavior:'smooth',block:'start'});
        el.style.background='rgba(99,102,241,.15)';el.style.borderColor='var(--accent)';
...
function renderRingkasan(){
  const cont=document.getElementById('ringkasan-c');if(!cont)return;cont.innerHTML='';
  ...
  WD.days.forEach((day,di)=>{
    ...
    day.items.forEach((item,ii)=>{
      ...
      if(!CK[iid(di,ii)]){
        ...
        const duO=du!==null?du-1:null;
        if(isPisang(item.b)&&duO!==null&&duO<=4&&duO>0) pisangItems.push({lbl,di,ii,orderDay:day.od});
        else if(duO===1) h1Items.push({lbl,di,ii,orderDay:day.od});
        else if(duO===2) h2Items.push({lbl,di,ii,orderDay:day.od});
      }
    });
  });
  const pct=tot>0?Math.round(dn/tot*100):0;
  ...
  grid.innerHTML=`
    <div class="sum-card ${h1Items.length?'red':''}" onclick="switchTabToDay('checklist','${h1Items[0]?.orderDay||''}')">
      <div class="sum-val">${h1Items.length}</div><div class="sum-lbl">Belum order H-1</div>
      ${h1Items.length?`<div class="sum-detail">${h1Items.slice(0,3).map(x=>x.lbl).join(', ')}${h1Items.length>3?` +${h1Items.length-3} lagi`:''}</div>`:''}
    </div>
    <div class="sum-card ${h2Items.length?'orange':''}" onclick="switchTabToDay('checklist','${h2Items[0]?.orderDay||''}')">
      <div class="sum-val">${h2Items.length}</div><div class="sum-lbl">Perlu order H-2</div>
    ...
    <div class="sum-card ${pisangItems.length?'yellow':''}" onclick="switchTab('checklist')">
      <div class="sum-val">${pisangItems.length}</div><div class="sum-lbl">🍌 Pisang H-4</div>
    ...
    <div class="sum-card ${pct===100?'green':''}" onclick="switchTab('checklist')">
      <div class="sum-val">${dn}/${tot}</div><div class="sum-lbl">Diorder (${pct}%)</div>
    </div>`;
  cont.appendChild(grid);
  if(h1Items.length){const a=document.createElement('div');a.className='sum-alert red';a.onclick=()=>switchTabToDay('checklist',h1Items[0]?.orderDay);a.innerHTML=`<div class="sum-alert-icon">🔴</div><div><div class="sum-alert-title">H-1 — Harus diorder HARI INI</div><div class="sum-alert-body">${h1Items.map(x=>x.lbl).join(' · ')}</div><div class="tap-hint">↑ tap untuk lihat checklist</div></div>`;cont.appendChild(a);}
  if(h2Items.length){const a=document.createElement('div');a.className='sum-alert orange';a.onclick=()=>switchTabToDay('checklist',h2Items[0]?.orderDay);a.innerHTML=`<div class="sum-alert-icon">🟠</div><div><div class="sum-alert-title">H-2 — Perlu diorder besok</div><div class="sum-alert-body">${h2Items.map(x=>x.lbl).join(' · ')}</div><div class="tap-hint">↑ tap untuk lihat checklist</div></div>`;cont.appendChild(a);}
  if(pisangItems.length){const a=document.createElement('div');a.className='sum-alert yellow';a.onclick=()=>switchTabToDay('checklist',pisangItems[0]?.orderDay);a.innerHTML=`<div class="sum-alert-icon">🍌</div><div><div class="sum-alert-title">Pisang — pesan H-4</div><div class="sum-alert-body">${pisangItems.map(x=>x.lbl).join(' · ')}</div><div class="tap-hint">↑ tap untuk lihat checklist</div></div>`;cont.appendChild(a);}
  if(!h1Items.length&&!h2Items.length&&!pisangItems.length){const a=document.createElement('div');a.className='sum-alert green';a.innerHTML=`<div class="sum-alert-icon">✅</div><div><div class="sum-alert-title" style="color:var(--green)">Semua order on track!</div></div>`;cont.appendChild(a);}
}
...
function refreshActive(){
  ...
  if(curTab==='rekap')     renderRekap();  // FIX
  updateProgress();
}
function updateProgress(){
  ...
  const ring=document.getElementById('ring-fill');if(ring)ring.style.strokeDashoffset=81.7*(1-pct/100);
...
async function handleImport(evt){
  ...
  try{
    ...
    rows.forEach(row=>{
      const a=String(row[0]||'').trim(),b=String(row[1]||'').trim(),c=row[2],d=String(row[3]||'').trim(),e=row[4];
      if(DAY.test(a)&&!b){
        const formatted=fmtTgl(a),od=orderDay(a);
        const tm=a.match(/(\d{1,2})\s+(\w+)\s+(\d{4})/);let os='';
    ...
    const subEl=document.getElementById('dapur-sub');
    if(subEl)subEl.textContent=fmtTgl(WD.days[0].n);
...
function rItems(di){
  ...
  WD.days[di].items.forEach((item,ii)=>{
    ...
    const fBahan=`<input type="text" class="inp-bahan" value="${escH(item.b)}" placeholder="Nama bahan" onblur="upItB(${di},${ii},this.value)" oninput="upIt(${di},${ii},'b',this.value)">`;
    const fQty=`<input type="text" class="inp-num" value="${item.q?fNum(item.q):''}" placeholder="Qty" onblur="upItN(${di},${ii},'q',this.value)" onfocus="this.value=this.value.replace(/[.]/g,'')">`;
    const fSat=`<input type="text" class="inp-sat" value="${escH(normSat(item.s||''))}" placeholder="kg" oninput="upIt(${di},${ii},'s',this.value)">`;
    const fH=`<input type="text" class="inp-num${hMissing?' harga-missing':''}" value="${item.h?fNum(item.h):''}" placeholder="${hMissing?'⚠ harga':'harga'}" onblur="upItN(${di},${ii},'h',this.value)" onfocus="this.value=this.value.replace(/[.]/g,'')">`;
    const fHB=`<input type="text" class="inp-num inp-hbaru${item.hb?' changed':''}" value="${item.hb?fNum(item.hb):''}" placeholder="ubah harga" onblur="upItN(${di},${ii},'hb',this.value)" onfocus="this.value=this.value.replace(/[.]/g,'')">`;
    const fKet=`<div class="ket-wrap" id="ketwrap-${di}-${ii}">
      ...
      ${isCustomKet?`<input type="text" class="inp-ket-custom" value="${escH(item.ket)}" placeholder="nama dapur..." oninput="upIt(${di},${ii},'ket',this.value)" onblur="saveLocal()">`:`<input type="text" class="inp-ket-custom" style="display:none" placeholder="nama dapur..." oninput="upIt(${di},${ii},'ket',this.value)">`}
      </div>`;
    ...
    c.appendChild(row);
    const sub=document.createElement('div');sub.className='item-subtotal'+(ck?' is-ck':'');
    sub.innerHTML=`<div style="position:relative">
        <button class="kat-btn${isCustomKat?' custom':''}" id="kat-btn-${di}-${ii}"
          onclick="showKatPicker(${di},${ii},'${escH(item.b)}')">${cfg.e} ${cfg.l}${isCustomKat?' ✏️':''}</button>
      </div>
...
function rmIt(di,ii){
  const nama=WD.days[di].items[ii].b||'item ini';
  if(!confirm(`Hapus "${nama}"?`))return;  // konfirmasi hapus
  WD.days[di].items.splice(ii,1);delete CK[iid(di,ii)];saveLocal();renderInput(false);
}
...
function showKatPicker(di,ii,name){
  ...
  pop.innerHTML=`<div class="kat-picker-ttl">Kategori: <strong>${escH(name)}</strong></div>
  ...
  btn.parentNode.style.position='relative';btn.parentNode.appendChild(pop);
...
async function applyKat(di,ii,name,kat){
  ...
  toast(`✓ "${name}" → ${CC[kat].e} ${CC[kat].l}`);rItems(di);
}
...
function renderOrderH(){
  const{G,seq}=buildOG();const cont=document.getElementById('order-h-c');cont.innerHTML='';
  seq.forEach(key=>{
    ...
    block.innerHTML=`<div class="msg-hdr"><div class="msg-hdr-l">◎ Order ${escH(grp.label)}</div><span class="msg-for">untuk ${escH(grp.menu)}</span></div>
...
function renderCL(){
  ...
  seq.forEach(key=>{
    ...
    block.innerHTML=`<div class="od-title" id="od-${key}">◎ Order ${escH(grp.label)} — untuk ${escH(grp.menu)}<span class="chip ${isAll?'green':''}" style="margin-left:auto">${dN}/${items.length}</span></div>`;
    items.forEach(item=>{
...
function renderRekap(){
  ...
  if(head){
    ...
    const thBahan=document.createElement('th');thBahan.style.textAlign='left';thBahan.style.minWidth='120px';thBahan.textContent='Bahan';head.appendChild(thBahan);
    WD.days.forEach((d,i)=>{const th=document.createElement('th');th.id=`rh${i}`;th.style.minWidth='62px';th.style.whiteSpace='pre';th.textContent=shortDay(d.n);head.appendChild(th);});
    const thTot=document.createElement('th');thTot.className='th-total';thTot.style.minWidth='70px';thTot.textContent='Total';head.appendChild(thTot);
  }
  const map={};
  ...
  if(!multi.length){tb.innerHTML=`<tr><td colspan="${3+WD.days.length}" style="text-align:center;padding:2rem;color:var(--text3)">Tidak ada bahan berulang.</td></tr>`;return;}
  ...
  multi.forEach(item=>{
    ...
    const idsPerDay={};
    WD.days.forEach((day,di)=>{day.items.forEach((it,ii)=>{if(it.b.toLowerCase().trim()!==item.d.toLowerCase().trim())return;if(!idsPerDay[day.n])idsPerDay[day.n]=[];idsPerDay[day.n].push(iid(di,ii));});});
    ...
    const cells=WD.days.map(d=>{
      const v=item.pd[d.n];const dayIds=idsPerDay[d.n]||[];const dayCk=dayIds.length>0&&dayIds.every(id=>CK[id]);
      if(v){tot+=v;if(dayCk)return`<td class="nv" style="opacity:.38;text-decoration:line-through;color:var(--text3)">${fNum(v)}</td>`;return`<td class="nv">${fNum(v)}</td>`;}
      return`<td class="nd">—</td>`;
    }).join('');
    const idsStr=JSON.stringify(item.ids).replace(/"/g,"'");
    tr.innerHTML=`<td style="text-align:center;width:32px">
        <div class="chk-sm ${allCk?'on':''}" onclick="toggleGroup(${idsStr})" style="margin:0 auto">${allCk?'✓':''}</div>
      </td>
      <td style="font-weight:500">${escH(item.d)}<div style="font-size:10px;color:var(--text3);margin-top:1px">${escH(normSat(item.s||''))}</div></td>
      ${cells}
...
function exportRekap(format){
  const{G,seq}=buildOG();
  // Kumpulkan semua bahan berulang
  const map={};
  WD.days.forEach((day,di)=>day.items.forEach((item,ii)=>{
    if(!item.b)return;const k=item.b.toLowerCase().trim();
    if(!map[k])map[k]={d:item.b,s:item.s,cat:gc(item.b),pd:{}};
    if(item.q)map[k].pd[day.n]=(map[k].pd[day.n]||0)+item.q;
  }));
  const rows=Object.values(map).filter(v=>Object.keys(v.pd).length>=2);
  rows.sort((a,b)=>CO.indexOf(a.cat)-CO.indexOf(b.cat)||a.d.localeCompare(b.d));
  const dayNames=WD.days.map(d=>d.n);
  const header=['Bahan','Satuan',...dayNames.map(n=>shortDay(n).replace('\n',' ')),'Total'];
  const data=[header];
  let lastCat=null;
  rows.forEach(item=>{
    if(item.cat!==lastCat){
      lastCat=item.cat;
      data.push([`── ${CC[item.cat].l} ──`,'','','','']);
    }
    let tot=0;
    const row=[item.d,normSat(item.s||'')];
    dayNames.forEach(dn=>{const v=item.pd[dn]||0;tot+=v;row.push(v||'');});
    row.push(tot);
    data.push(row);
  });
  ...
  if(format==='csv'){
    ...
    a.download=`rekap-${curDapurCfg?.name||'dapur'}-${new Date().toLocaleDateString('id-ID').replace(/\//g,'-')}.csv`;
    a.click();toast('✓ Exported ke CSV');
  } else {
    // Excel via XLSX library (sudah dimuat)
    const ws=XLSX.utils.aoa_to_sheet(data);
    // Lebar kolom
    ws['!cols']=[{wch:24},{wch:8},...dayNames.map(()=>({wch:10})),{wch:10}];
    const wb=XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb,ws,'Rekap');
    XLSX.writeFile(wb,`rekap-${curDapurCfg?.name||'dapur'}-${new Date().toLocaleDateString('id-ID').replace(/\//g,'-')}.xlsx`);
    toast('✓ Exported ke Excel');
  }
}
...
// ═══════════════════════════════════════════════════
function renderStok(){
  const cont=document.getElementById('stok-c');if(!cont)return;cont.innerHTML='';
  // Seksi 1: Dapur
  const sec1=document.createElement('div');sec1.className='stok-section';
  sec1.innerHTML=`<div class="stok-section-header">📦 Stok Dapur Ini</div>
    <div class="stok-section-body">
      <div class="stok-hdr-simple"><span class="sh-bahan">Bahan</span><span class="sh-jml">Jumlah</span><span class="sh-sat">Satuan</span><span></span></div>
      <div id="stok-items"></div>
      <button class="btn-add" onclick="addStok()" style="margin-top:4px">+ Tambah stok dapur</button>
    </div>`;
  cont.appendChild(sec1);
  const si=document.getElementById('stok-items');
  if(!STOK.length)si.innerHTML='<p class="empty" style="padding:.75rem 0;font-size:12px">Belum ada stok.</p>';
  STOK.forEach((item,i)=>{
    const row=document.createElement('div');row.className='stok-row-simple';
    row.innerHTML=`<input type="text" class="sh-bahan" value="${escH(item.b)}" placeholder="Nama bahan" onblur="upStokB(${i},this.value)" oninput="upStok(${i},'b',this.value)">
      <input type="text" class="inp-num sh-jml" value="${item.jml!=null?fNum(item.jml):''}" placeholder="0" onblur="upStokN(${i},'jml',this.value)" onfocus="this.value=this.value.replace(/[.]/g,'')">
      <input type="text" class="inp-sat sh-sat" value="${escH(normSat(item.s||''))}" placeholder="kg" oninput="upStok(${i},'s',this.value)">
      <button class="btn-x" onclick="rmStok(${i})">✕</button>`;
    si.appendChild(row);
  });
  // Seksi 2: Koperasi
  const sec2=document.createElement('div');sec2.className='stok-section';
  sec2.innerHTML=`<div class="stok-section-header" style="background:rgba(99,102,241,.1);border-color:rgba(99,102,241,.15)">
      🏪 Stok Koperasi
      <span style="font-size:10px;color:var(--text3);font-weight:400;margin-left:auto">Dibagikan ke semua dapur</span>
    </div>
    <div class="stok-section-body">
      <div class="stok-hdr-simple"><span class="sh-bahan">Bahan</span><span class="sh-jml">Jumlah</span><span class="sh-sat">Satuan</span><span></span></div>
      <div id="stok-kop-items"></div>
      <button class="btn-add" onclick="addStokKop()" style="margin-top:4px">+ Tambah stok koperasi</button>
    </div>`;
  cont.appendChild(sec2);
  const sk=document.getElementById('stok-kop-items');
  if(!STOK_KOPERASI.length)sk.innerHTML='<p class="empty" style="padding:.75rem 0;font-size:12px">Belum ada stok koperasi.</p>';
  STOK_KOPERASI.forEach((item,i)=>{
    const row=document.createElement('div');row.className='stok-row-simple';
    row.innerHTML=`<input type="text" class="sh-bahan" value="${escH(item.b)}" placeholder="Nama bahan" onblur="upStokKopB(${i},this.value)" oninput="upStokKop(${i},'b',this.value)">
      <input type="text" class="inp-num sh-jml" value="${item.jml!=null?fNum(item.jml):''}" placeholder="0" onblur="upStokKopN(${i},'jml',this.value)" onfocus="this.value=this.value.replace(/[.]/g,'')">
      <input type="text" class="inp-sat sh-sat" value="${escH(normSat(item.s||''))}" placeholder="kg" oninput="upStokKop(${i},'s',this.value)">
      <button class="btn-x" onclick="rmStokKop(${i})">✕</button>`;
    sk.appendChild(row);
  });
}
function addStok(){STOK.push({b:'',jml:null,s:'kg'});saveLocal();renderStok();}
function rmStok(i){if(!confirm('Hapus stok ini?'))return;STOK.splice(i,1);saveLocal();renderStok();}
...
function addStokKop(){STOK_KOPERASI.push({b:'',jml:null,s:'kg'});dbSaveKoperasiStok();renderStok();}
function rmStokKop(i){if(!confirm('Hapus stok koperasi ini?'))return;STOK_KOPERASI.splice(i,1);dbSaveKoperasiStok();renderStok();}
function upStokKop(i,f,v){STOK_KOPERASI[i][f]=v;dbSaveKoperasiStok();}
function upStokKopB(i,v){STOK_KOPERASI[i].b=toTitle(v);dbSaveKoperasiStok();renderStok();}
function upStokKopN(i,f,v){const c=String(v).replace(/\./g,'').replace(/[^0-9]/g,'');STOK_KOPERASI[i][f]=c?parseInt(c):null;dbSaveKoperasiStok();renderStok();}

// ═══════════════════════════════════════════════════
// HISTORI
// ═══════════════════════════════════════════════════
async function rHist(){
  const c=document.getElementById('hist-c');c.innerHTML='<p class="empty">Memuat...</p>';
...
async function loadH(id){const res=await dbLoadWeek(id);if(!res?.week)return;WD=res.week.data;CK=res.ck?.ck_data||{};STOK=res.stok?.items||[];curWeekId=id;saveLocal();switchTab('input');renderInput();toast('✓ Dimuat');}
async function dupH(id){const res=await dbLoadWeek(id);if(!res?.week)return;WD=JSON.parse(JSON.stringify(res.week.data));CK={};STOK=[];curWeekId=null;saveLocal();switchTab('input');renderInput();toast('Duplikat berhasil');}
async function dupFirst(){const rows=await dbLoadWeeks(curDapurId);if(!rows.length){toast('Belum ada histori.');return;}await dupH(rows[0].id);}
